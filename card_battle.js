...
const STATUS_DEFS={
  poison:{name:'毒',desc:'ターン終了時に数値分のダメージを受ける'},
  weak:{name:'弱体化',desc:'攻撃の与ダメージが25%減少する'},
  block:{name:'ブロック',desc:'敵の攻撃ダメージを数値分だけ防ぐ'}
};
...
function applyStatusEffects(){
  ...
  if(s.weak>0) s.weak--;
  if(state.playerStatus.poison>0){glowIcon('p-poison');state.playerHP=Math.max(0,state.playerHP-state.playerStatus.poison);addLog(`毒で${state.playerStatus.poison}ダメージ！`,'psn');state.playerStatus.poison--;shakeUnit('p-unit')}
  if(state.playerStatus.bleed>0){glowIcon('p-bleed');state.playerHP=Math.max(0,state.playerHP-state.playerStatus.bleed);addLog(`出血で${state.playerStatus.bleed}ダメージ！`,'dmg');shakeUnit('p-unit')}
}
...
function updateBattleUI(){
  ...
  const pp=el('p-poison');if(pp){pp.textContent=`☠${state.playerStatus.poison||0}`;pp.classList.toggle('show',(state.playerStatus.poison||0)>0);}
  const pb=el('p-bleed');if(pb){pb.textContent=`🩸${state.playerStatus.bleed||0}`;pb.classList.toggle('show',(state.playerStatus.bleed||0)>0);}
...
function updateTopBar(){
  ...
  if(state.relics.length){
    const relicsHtml = state.relics.map(r => 
      `<span id="relic-icon-${r.id}" class="relic-icon" onmouseenter="showGameTooltip(event, 'relic', '${r.id}')" onmouseleave="hideGameTooltip()" onclick="toggleGameTooltip(event, 'relic', '${r.id}')">${r.icon}</span>`
    ).join('');
...
function showGameTooltip(e, type, id) {
  clearTimeout(tooltipTimer);
  let item = type === 'relic' ? RELIC_DEFS.find(r => r.id === id) : STATUS_DEFS[id];
  if(!item) return;
  const tt = el('game-tooltip');
  const descHtml = item.desc.replace(/(\d+%?)/g, '<strong>$1</strong>');
  tt.innerHTML = `<h3>${item.name}</h3><p>${descHtml}</p>`;
...
function toggleGameTooltip(e, type, id) {
  const tKey = type + '_' + id;
  if (activeTooltipId === tKey && el('game-tooltip').classList.contains('show')) {
    hideGameTooltip();
    activeTooltipId = null;
  } else {
    showGameTooltip(e, type, id);
    activeTooltipId = tKey;
  }
}

function glowIcon(id) {
  const icon = el(id);
  if(icon) {
